...
namespace CleanTeeth.Tests.Domain.Entities
{
    ...
    public class AppointmentTests
    {
        ...
        [TestMethod]
        public void Constructor_ValidAppointment_StatusIsSheduled()
        {
        ...
        [TestMethod]
        [ExpectedException(typeof(BusinessRuleException))]
        public void Constructor_CancellingAppointment_ThrowsIfStatusIsNotScheduled()
        {
            var appointment = new Appointment(_patientId, _dentistId, _dentalOfficeId, _interval);
            appointment.Cancel();
            appointment.Cancel();
        ...
        [TestMethod]
        public void Constructor_CopmlitingAppointment_ChangesStatusToCompleted()
        {
            var appointment = new Appointment(_patientId, _dentistId, _dentalOfficeId, _interval);
            appointment.Complete();
            Assert.AreEqual(AppointmentStatus.Copmleted, appointment.Status);
        }

        [TestMethod]
        [ExpectedException(typeof(BusinessRuleException))]
        public void Constructor_ComplitingAppointment_ThrowsIfStatusIsNotScheduled()
        {
            var appointment = new Appointment(_patientId, _dentistId, _dentalOfficeId, _interval);
            appointment.Cancel();
            appointment.Complete();
